...
router.put('/:userId', authorize(), updateSchema, update);
router.delete('/:userId', authorize(), _delete);

// router.get('/:userId/log-events', authorize(), getLogEvents);

router.use('/:userId/log-events', usersEventsController);
...
function getLogEvents(req, res, next) {
    // users can get logs of their own user and admins can get logs of any user
	if (req.params.userId !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    userService.getLogEvents(req.params.userId)
        .then(user => user ? res.json(user) : res.sendStatus(404))
        .catch(next);
}
