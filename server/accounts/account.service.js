...
async function getLogEvents(accountId) {
    const account = await getAccountWithLogs(accountId);
    return logEventsDetails(account);
}
...
async function getAccountWithLogs(accountId) {
    if (!db.isValidId(accountId)) throw 'Account not found';
...
function logEventsDetails(account) {
    const { id, email, logEvents } = account;
